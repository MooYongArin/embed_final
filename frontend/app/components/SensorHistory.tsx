...
export function SensorHistory({ history }: SensorHistoryProps) {
  const ultrasonicData = history.map((data, index) => ({
    index,
    value: data.ultrasonic.distance,
  }));

  const fireData = history.map((data, index) => ({
    index,
    temp: data.fire.temperature,
    infrared: data.fire.infraredLevel,
  }));

  const tiltData = history.map((data, index) => ({
    index,
    value: Math.abs(data.tilt.angle),
  }));
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-slate-300 text-sm">Temperature</h3>
          <span className="text-xs text-slate-500">Last 40s</span>
        </div>
        <ResponsiveContainer width="100%" height={80}>
          <LineChart data={fireData}>
            <YAxis hide domain={[15, 40]} />
            <XAxis hide dataKey="index" />
            <Line
              type="monotone"
              dataKey="temp"
      ...
      <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-slate-300 text-sm">Tilt Angle</h3>
          <span className="text-xs text-slate-500">Last 40s</span>
        </div>
        <ResponsiveContainer width="100%" height={80}>
